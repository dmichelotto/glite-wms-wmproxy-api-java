...
import org.gridsite.www.namespaces.delegation_2.WMProxyStub.NewProxyReq;
...
*/
	...
	*/
	public static void runTest ( String url, String proxyFile, String certsPath ) throws org.glite.wms.wmproxy.BaseException  {
		NewProxyReq proxyReq;
		String proxy = "" ;
		String delegId = "";
		WMProxyAPI client = null;

		System.out.println ("TEST : New-GridSite Delegation");
		System.out.println ("************************************************************************************************************************************");
		...
		// Proxy Request
		System.out.println ("\nCalling getNewProxyReq service\n");
		proxyReq = client.getNewProxyReq ( );
		System.out.print ("Reading result ....");
		proxy = proxyReq.getProxyRequest( );
		delegId = proxyReq.getDelegationID( );
		System.out.println ("\nproxy:\n" + proxy);
		System.out.println ("Delagation ID =[" + delegId + "]" );
		System.out.println ("\n===================================================");
		System.out.println ("\nDelegating Credential .....");
		// Put prxoy
		client.grstPutProxy (delegId, proxy);
		// end
		System.out.println ("\nYour credential has been successfully delegated with delegationId: " + delegId);
		System.out.println ("\nEnd of the test\n");
	}
	/**
	*	main
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			if ((args == null) || (args.length < 2)){
			  ...
			  throw new Exception ("error: too many parameters\nUsage: java <package>.<class> <WebServices URL> <Delegation-ID> [CAs paths (optional)] )");
			}
			url = args[0];
			proxyFile = args[1];
			if (args.length == 3) {
